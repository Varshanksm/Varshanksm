# I'm Ragavarshan KSM 👋

**Computer Science & Business Systems Student**

**Java · Backend Development · Machine Learning · DSA**

I'm a B.Tech Computer Science and Business Systems student at  
**Thiagarajar College of Engineering, Madurai.**

Interested in **Data Structures & Algorithms, Backend Development,  
Machine Learning, Databases, and Problem Solving.**

[![LeetCode](https://img.shields.io/badge/LeetCode-orange?style=flat&logo=leetcode&logoColor=white)](https://leetcode.com/u/Ragavarshan/)
[![GitHub](https://img.shields.io/badge/GitHub-black?style=flat&logo=github&logoColor=white)](https://github.com/Varshanksm)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/varshanmdu27)
[![Email](https://img.shields.io/badge/Email-red?style=flat&logo=gmail&logoColor=white)](mailto:ragavarshan2006@gmail.com)
---

## 👨‍💻 About Me

- 🎓 B.Tech Computer Science and Business Systems @ Thiagarajar College of Engineering
- 💻 Interested in **Data Structures & Algorithms and Problem Solving**
- ☕ Programming primarily with **Java and SQL**
- 🤖 Interested in **Machine Learning and Generative AI**
- 🌐 Experienced in **Backend Development and REST APIs**
- 🗄️ Working with **MongoDB and MySQL**
- 📊 Interested in **Data Analytics and Data Engineering**
- 🏆 1st Prize – Code Hunt Challenge

---

## 🛠️ Tech Stack

### Languages

![Java](https://img.shields.io/badge/Java-orange?style=flat&logo=openjdk&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-blue?style=flat)

### Backend

![Node.js](https://img.shields.io/badge/Node.js-green?style=flat&logo=nodedotjs&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-black?style=flat&logo=express&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20APIs-blue?style=flat)

### Databases

![MongoDB](https://img.shields.io/badge/MongoDB-green?style=flat&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-blue?style=flat&logo=mysql&logoColor=white)

### Data & AI

![Machine Learning](https://img.shields.io/badge/Machine%20Learning-purple?style=flat)
![Generative AI](https://img.shields.io/badge/Generative%20AI-black?style=flat)
![Excel](https://img.shields.io/badge/Excel-green?style=flat&logo=microsoftexcel&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-blue?style=flat&logo=tableau&logoColor=white)

### Frontend

![HTML](https://img.shields.io/badge/HTML5-orange?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-blue?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-yellow?style=flat&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-blue?style=flat&logo=react&logoColor=white)

### Tools

![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ%20IDEA-black?style=flat&logo=intellijidea&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-blue?style=flat&logo=visualstudiocode&logoColor=white)

---

## 🚀 Projects

### 🎪 TCE Event Management Portal

**Node.js · Express.js · MongoDB · REST APIs**

- Developed a multi-role event management portal.
- Implemented event creation, registration, user management and notifications.
- Designed MongoDB schemas for users, event registrations and notifications.
- Implemented role-based functionality.

---

### 🛡️ Phishing Detector Chrome Extension

**Machine Learning · Logistic Regression · Chrome Extension API**

- Developed a real-time phishing URL detection Chrome extension.
- Used 10+ URL features including entropy, HTTPS, subdomain depth and suspicious keywords.
- Integrated Logistic Regression to classify URLs as **Safe, Suspicious, or High Risk**.
- Built a domain trust system using 50+ verified domains.

---

### 🏠 Hostel Complaint Management System

**React.js · Firebase · Team Project**

- Developed a hostel complaint management system for students and administrators.
- Implemented authentication and role-based access.
- Added real-time complaint tracking.
- Built an admin dashboard for monitoring and resolving complaints.

---

## 💼 Experience

### SDE Intern — Blaze Web Services

**June 2025 – July 2025 · Madurai**

- Developed frontend components and UI enhancements using HTML, CSS and JavaScript.
- Improved website layouts and UI consistency across multiple pages.

---

## 🏆 Achievements

🥇 **1st Prize — Code Hunt Challenge**

Department-level coding competition.

### Leadership & Activities

- Volunteer — **Youth Red Cross (YRC)**
- Organized and managed a college-level blood donation camp.

---

## 📜 Certifications

🎓 **AWS Academy Graduate – Data Engineering**

---

## 🎯 Current Focus

```text
DSA → Problem Solving → Backend Development → Machine Learning
                    ↓
              Building Projects
